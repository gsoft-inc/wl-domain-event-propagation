...
public class SubscriptionTopicValidatorTests
{
    ...
    {
        this._topicProviderMock.Setup(x => x.GetTopicValidationPattern(OrganizationTopic.Name)).Returns(OrganizationTopic.Pattern);
        this._topicProviderMock.Setup(x => x.GetTopicValidationPattern(SignupTopic.Name)).Returns(SignupTopic.Pattern);
        ...
        var subscriptionTopicValidator = new SubscriptionTopicValidator(optionsWrapper, new[] { this._topicProviderMock.Object });
